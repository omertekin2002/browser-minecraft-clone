...
Everything is procedural: the infinite terrain, biomes, caves and trees, all block textures
(albedo, normal/height and PBR material maps), the sounds, the clouds and the sky. There are no
asset files.
...
- Around 70 block types, including glass, water, ice, lava, torches, glowstone, sea lanterns,
  metal and gem blocks, wool, flowers, cactus and sugar cane.
...
- Creative mode (instant breaking, flight) and Survival mode (timed breaking with crack
  animation, no flight).
- Falling sand/gravel, plants that pop off when unsupported, water that flows into opened space.
- A first-person held block with swing animation, block-break particles, and procedural WebAudio
  (footsteps, digging, placing, wind, birds, crickets, muffled underwater audio).
- Worlds are saved automatically: edited chunks go to IndexedDB, and the player position goes to
  localStorage.
...
src/
  ...
  world/                  blocks, constants, World (chunk streaming), WorkerPool, worker
  ...
  render/                 Renderer (pipeline), ChunkMeshes (quad heap), FarTerrain, Entities,
                          Atmosphere, math, gl helpers
    textures/             procedural block texture generator
    shaders/              all GLSL (includes resolved at load time)
  ui/                     HUD, menus, block icons
```
