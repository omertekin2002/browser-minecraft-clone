...
/** Integer hash of 2D coordinates → uint32. */
...
/** Integer hash of 3D coordinates → uint32. */
export function hash3(x: number, y: number, z: number, seed: number): number {
  let h = (seed | 0) ^ Math.imul(x | 0, 0x27d4eb2d) ^ Math.imul(y | 0, 0x1b873593) ^ Math.imul(z | 0, 0x165667b1);
  h = Math.imul(h ^ (h >>> 15), 0x85ebca6b);
  h = Math.imul(h ^ (h >>> 13), 0xc2b2ae35);
  return (h ^ (h >>> 16)) >>> 0;
}
...
/** Classic simplex noise (Gustavson) with a seeded permutation table. Output ≈ [-1, 1]. */
export class SimplexNoise {
  ...
  fbm2(x: number, y: number, octaves: number, lacunarity = 2, gain = 0.5): number {
    ...
    for (let o = 0; o < octaves; o++) {
      sum += this.noise2D(x * f + o * 17.31, y * f - o * 9.73) * amp;
      norm += amp;
      amp *= gain;
      f *= lacunarity;
    }
  ...
  fbm3(x: number, y: number, z: number, octaves: number, lacunarity = 2, gain = 0.5): number {
    let sum = 0, amp = 1, norm = 0, f = 1;
    for (let o = 0; o < octaves; o++) {
      sum += this.noise3D(x * f + o * 17.31, y * f + o * 5.19, z * f - o * 9.73) * amp;
      norm += amp;
      amp *= gain;
      f *= lacunarity;
    }
